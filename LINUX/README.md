...
Я бы рекомендовал не переопределять rm глобально, а создать отдельный алиас:

```
# В ~/.bashrc
alias del='trash-put'        # Безопасное удаление
alias rm='rm -i'            # Обычный rm с подтверждением
alias rmf='\rm -rf'         # Настоящее удаление (когда точно уверены)
```
                           # \rm вызывает оригинальную команду, игнорируя алиасы
Как это спасло бы вашу ситуацию:
```
Если бы был настроен алиас
rm -R /home/silenzio/ros2_ws/install/
```

Вы бы потом просто сделали:
```
trash-restore
```
